...
func main() {
	...
	if *inputUnixSocketPath == "" && *inputTCPSocket == "" && *inputTLSSocket == "" {
		logger.Error("missing flag", "error", "missing input, must be one of: -input-unix, -input-tcp or -input-tls")
		os.Exit(1)
	}
	...
	if conf.CryptoPanKey == "" {
		logger.Error("cryptopan setup error", "error", "missing required setting 'cryptopan-key' in config", "configfile", *configFile)
		os.Exit(1)
	}
